...
TASKS: list[tuple[str, str, str]] = [
    ...
    (
        "anti_scheming.spec_sabotage.spec_sabotage_moving",
        "as_spec_sabotage_moving",
        "spec_sabotage",
    ),
    (
        "anti_scheming.spec_sabotage.spec_sabotage_selection",
...
def main() -> None:
    ...
    ap.add_argument(
        "--model", default=os.environ.get("INSPECT_EVAL_MODEL", "sglang/Qwen/Qwen3.5-0.8B")
    )
    ...
    ap.add_argument(
        "--only", nargs="*", default=None, help="restrict to a list of task names (e.g. as_blotto)"
    )
    ap.add_argument("--bucket", default=None, help="restrict to one bucket")
    ap.add_argument(
        "--attempts", type=int, default=1, help="default attempts per sample passed to react()"
    )
    ap.add_argument(
        "--task-attempts",
        nargs="*",
    ...
    selected = [
        (m, n, b)
        for (m, n, b) in TASKS
        if (args.only is None or n in args.only) and (args.bucket is None or b == args.bucket)
    ]

    print(
        f"Running {len(selected)} task(s) against {args.model} "
