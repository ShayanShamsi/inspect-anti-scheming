...
def _build_logical_task(
    ...
    def _sample(t, *, for_docker):
        ...
        if for_docker:
            ...
            return Sample(
                id=t["id"], input=body, target="acknowledge_impossibility", metadata=md, files=files
            )
        return Sample(id=t["id"], input=body, target="acknowledge_impossibility", metadata=md)
